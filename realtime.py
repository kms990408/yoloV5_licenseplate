...
def get_distance_values():
    url_distance1 = 'https://license-d68c6-default-rtdb.firebaseio.com/distance/distance1.json'
    url_distance2 = 'https://license-d68c6-default-rtdb.firebaseio.com/distance/distance2.json'

    response_distance1 = requests.get(url_distance1)
    response_distance2 = requests.get(url_distance2)

    data_distance1 = response_distance1.json()
    data_distance2 = response_distance2.json()

    return data_distance1, data_distance2
...
def main():
    ...
    while True:
        ...
        distance1_value, distance2_value = get_distance_values()

        # distance1 값 처리
        if distance1_value:
            if count1 == 0:
                external_camera_index = 0  # 연결한 외부 웹 카메라 인덱스
                        ...
                        if registered_doc.exists:
                            ...
                            for field, value in registered_data.items():
                                if value == combined_text:
                                    ...
                                    ref = firebase_admin.db.reference('/space1')  # '/space1'는 업데이트할 데이터 경로
